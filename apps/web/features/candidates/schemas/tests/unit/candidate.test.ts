...
describe('createCandidateSchema', () => {
  ...
  it('accepts an email with no portal-access decision (that lives outside the schema — see create-candidate.ts)', () => {
    const result = createCandidateSchema.safeParse({ ...valid, email: 'jane@example.com' });
    expect(result.success).toBe(true);
  });
});
