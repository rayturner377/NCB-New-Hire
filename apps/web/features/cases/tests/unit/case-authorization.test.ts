...
const { matchesClinicianAssignment, patientOwnsCase } = await import('../../case-authorization');

describe('matchesClinicianAssignment', () => {
  it('lets a clinician act on a case assigned to them', () => {
    expect(matchesClinicianAssignment({ role: 'clinician', id: 'usr_doctor_1' }, { assignedClinicianId: 'usr_doctor_1' })).toBe(true);
  });

  it('blocks a clinician from a case assigned to a different doctor', () => {
    expect(matchesClinicianAssignment({ role: 'clinician', id: 'usr_doctor_1' }, { assignedClinicianId: 'usr_doctor_2' })).toBe(false);
  });

  it('blocks a clinician from an unassigned case', () => {
    expect(matchesClinicianAssignment({ role: 'clinician', id: 'usr_doctor_1' }, { assignedClinicianId: null })).toBe(false);
  });

  it('never restricts non-clinician roles', () => {
    expect(matchesClinicianAssignment({ role: 'admin', id: 'usr_admin_1' }, { assignedClinicianId: 'usr_doctor_2' })).toBe(true);
    expect(matchesClinicianAssignment({ role: 'reviewer', id: 'usr_reviewer_1' }, { assignedClinicianId: null })).toBe(true);
    expect(matchesClinicianAssignment({ role: 'auditor', id: 'usr_auditor_1' }, { assignedClinicianId: 'usr_doctor_2' })).toBe(true);
  });

  it('lets a delegate act on a case assigned to the doctor they support', () => {
    expect(
      matchesClinicianAssignment(
  ...
  it('blocks a delegate with no doctor linked yet from every case, even an unassigned one', () => {
    expect(matchesClinicianAssignment({ role: 'delegate', id: 'usr_delegate_1', delegateForClinicianId: null }, { assignedClinicianId: null })).toBe(
      false
    );
  });
});
